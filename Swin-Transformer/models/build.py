...
from .swin_transformer_cope import CoPESwinTransformer
...
def build_model(config, is_pretrain=False):
    ...
    elif model_type == 'swin_cope':
        model = CoPESwinTransformer(img_size=config.DATA.IMG_SIZE,
                                    patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                    in_chans=config.MODEL.SWIN.IN_CHANS,
                                    num_classes=config.MODEL.NUM_CLASSES,
                                    embed_dim=config.MODEL.SWIN.EMBED_DIM,
                                    ...
                                    use_checkpoint=config.TRAIN.USE_CHECKPOINT,
                                    fused_window_process=config.FUSED_WINDOW_PROCESS,
                                    npos_max=config.MODEL.SWIN_COPE.N_POSMAX, 
                                    cope_k=config.MODEL.SWIN_COPE.K, 
                                    cope_q=config.MODEL.SWIN_COPE.Q, 
                                    cope_v=config.MODEL.SWIN_COPE.V)
    elif model_type == 'swin_cope2D':
        model = CoPESwinTransformer(img_size=config.DATA.IMG_SIZE,
                                    patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                    in_chans=config.MODEL.SWIN.IN_CHANS,
                                    num_classes=config.MODEL.NUM_CLASSES,
                                    ...
                                    use_checkpoint=config.TRAIN.USE_CHECKPOINT,
                                    fused_window_process=config.FUSED_WINDOW_PROCESS,
                                    npos_max=config.MODEL.SWIN_COPE2D.N_POSMAX, 
                                    cope_k=config.MODEL.SWIN_COPE2D.K, 
                                    cope_q=config.MODEL.SWIN_COPE2D.Q, 
                                    cope_v=config.MODEL.SWIN_COPE2D.V)
