...
def compute_axial_cis(dim: int, end_x: int, end_y: int, theta: float = 100.0):
    ...
    freqs_x = torch.outer(t_x, freqs_x)  # 内积，定义角度
    freqs_y = torch.outer(t_y, freqs_y)
    freqs_cis_x = torch.polar(torch.ones_like(freqs_x), freqs_x)  # 转极坐标
...
def apply_rotary_emb(
    ...
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2))  # (B, heads, patch, embed)
    xk_ = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))
    freqs_cis = reshape_for_broadcast(freqs_cis, xq_)  # (1, heads, patch, embed)
...
class CoPE(nn.Module):
    ...
    def forward(self, query, attn_logits, key):
...
@register_model
def rope_mixed_deit_small_patch4_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
...
@register_model
def rope_mixed_ape_deit_small_patch4_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = rope_vit_models(
        img_size = img_size, patch_size=4, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), block_layers=RoPE_Layer_scale_init_Block, Attention_block=RoPEAttention,
        rope_theta=10.0, rope_mixed=True, use_ape=True, **kwargs)
